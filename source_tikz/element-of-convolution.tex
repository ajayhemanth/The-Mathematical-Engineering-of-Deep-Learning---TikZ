...
\node [draw,very thick, circle, minimum size=1cm, right of = node-x22, node distance=6cm] (circle1) {\Large $\odot$};

\node [draw, very thick, circle, minimum size=1cm, right of = circle1, node distance=4cm] (circle2) {\Large $\sum$};

\draw[-Triangle, very thick, draw=black!50] (node-x32)+(7mm,0) -- ([xshift=-2mm]circle1.west);

\draw[-Triangle, very thick, draw=black!50] (circle1)+(7mm,0) -- ([xshift=-2mm]circle2.west);

\draw[-Triangle, very thick, draw=black!50] (node-w32)+(7mm,0) -- ([xshift=-2mm]node-y12.west) node[pos=.5, above] {Flipping};

\draw[-Triangle, very thick, draw=black!50] (node-y23)+(0,-7mm) -- ([yshift=2mm]circle1.north);

\draw[-Triangle, very thick, draw=black!50] (circle2)+(7mm,0) --  node[pos=1.3] {\Large $z_{1,1}$} ++(3cm,0);
