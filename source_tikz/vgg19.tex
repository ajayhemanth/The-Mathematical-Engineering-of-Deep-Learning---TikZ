...
\cube{17}{3.3}{.5}{.8}{1}{-1.3}{green0}{green1}

\node[style={yslant=0.9,xslant=0}] at (-3,-2,-6) { \Huge $x$ };

\node[] (first) at (-1.5,8,-3) {\Large $3 \times 224 \times 224$};

\draw[decorate, decoration={calligraphic brace,amplitude=5pt}, pen colour={black!50}, thick] (2.2,8.5) -- (3,8.5) node[above right =.25cm and -1.5cm]{\Large $64 \times 224 \times 224$ };

\draw [-Triangle, very thick, draw=black!50] (0.1,-2,-3) -- node[pos=-.4] { \Large $64 \times 112 \times 112$ } ++(0,1.3);

\draw[decorate, decoration={calligraphic brace,amplitude=5pt}, pen colour={black!50}, thick] (3.5,7.7) -- (4.4,7.7) node[above right =.25cm and -1cm]{\Large $128 \times 112 \times 112$ };

\draw [-Triangle, very thick, draw=black!50] (1.9,-1,-3) -- node[pos=-.4] { \Large $128 \times 56 \times 56$ } ++(0,.9);

\draw[decorate, decoration={calligraphic brace,amplitude=5pt}, pen colour={black!50}, thick] (5,7) -- (8.2,7) node[above left =.25cm and 0cm]{\Large $256 \times 56 \times 56$};

\draw [-Triangle, very thick, draw=black!50] (6,-1.2,-3) -- node[below left = .85 and -.85] { \Large $256 \times 28 \times 28$ } ++(-0,1.7);

\draw [Triangle-, draw=red, line width=1mm] (8.1, 1.8) -- node[pos=1.15, text=red] { \Large Layer 12 } ++(-0,-3);

\draw[decorate, decoration={calligraphic brace,amplitude=5pt}, pen colour={black!50}, thick] (8.9,6.2) -- (12.8,6.2) node[above left =.25cm and .7cm]{\Large $512 \times 28 \times 28$};

\draw [-Triangle, very thick, draw=black!50] (11,0,-3) -- node[pos=-.4] { \Large $512 \times 14 \times 14$ } ++(0,1.1);

\draw[decorate, decoration={calligraphic brace,amplitude=5pt}, pen colour={black!50}, thick] (13.5,5.4) -- (17.4,5.4) node[above left =.25cm and .5cm]{\Large $512 \times 14 \times 14$ };

\draw [-Triangle, very thick, draw=black!50] (16,.1,-3) -- node[pos=-.2] { \Large $512 \times 7 \times 7$ } ++(0,1.6);
...
\foreach \j in {1,...,4}
{
    ...
    \node[circle, draw, minimum size = .5cm, fill=blue1 ] (column-3\j) at (22, \pj+2) {};
}
  ...
  \foreach \i in {1,...,6}{
    \foreach \j in {1,...,6}{
        ...
        \ifthenelse{\j < 5}
        {
            ...
            \draw [-Triangle, draw=black!50] (column-3\j) -- node[pos=1.7, text=black!50] {\Large  $\hat{y}_{\outs}$} ++(1,0);
        }
        {}
    }
  }
