...
\node [draw, circle, minimum size=1cm, right of = node-x22, node distance=6cm] (circle1) {$\odot$};
...
\draw[-Triangle, draw=black!50] (node-w32)+(7mm,0) -- ([xshift=-2mm]node-y12.west) node[pos=.5, above] {Flipping};
\draw[-Triangle, draw=black!50] (node-y23)+(0,-7mm) -- ([yshift=2mm]circle1.north);
