...
\newcommand\fullthing[3]
{
    ...
    \draw[-Triangle] (block#1) -- (unit1b#1) node[pos=.2, above]{\Large #3};
...
\foreach \i in {1,...,7}
{
    \draw[-Triangle,thick] ([xshift=5mm]block1.east) -- ([xshift=-6mm]top\i-b1);
    \draw[-Triangle,thick] ([xshift=5mm]block1.east) -- ([xshift=-6mm]bottom\i-b1);
}
