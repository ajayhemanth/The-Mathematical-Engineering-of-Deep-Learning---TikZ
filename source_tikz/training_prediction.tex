...
\node[scale = .25] (inputpic) at (18,0) {\includegraphics {images/earth.jpeg}};
...
\path (model) edge [-stealth,out=270,in=270,draw=green1, line width=1mm] node [midway, anchor=center, fill=white] {$\hat{y}$} (inputpic);

\path (model) edge [-stealth,out=90,in=90,draw=green1, line width=1mm] node [midway, anchor=center, fill=white] {$ [ -\hspace{.5cm}  x^* \hspace{.5cm} - ] $} (inputpic);

\path (inputpic) edge [-stealth,dashed, out=80,in=100,draw=green1, line width=1mm] node [] {} (training);
